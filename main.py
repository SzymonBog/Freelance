...
webserverworm 80 - unlock port 80 using 'WevServerWorm.exe'
...
def createTables(cursor: mydb.cursor(), mydb) -> None:  # not finished{almost :) }
    # ---------------------------------------------
    # Only for players PC
    ...
    try:
        ...
        if pageNumber == 1:
            ...
            txt = "quit - exits the program\nresetDB - resets all data in database(loose all progress)[CAREFUL!!!!!!!]\n" \
                  ...
                  "nano {filename} - creates an empty file and allows to write in it"
            number = 5
        else:
...
def sshCrack(currentIp):  # not finished
    cursor.execute("select name, extension, content from files where ip='70.183.159.171'")
    files = cursor.fetchall()
    for f in files:
        if f[0] == 'SSHCrack22' and f[1] == 'exe' and f[2] == 'content':
            print("ok")
            quitGame()
            break
